package com.avimax.backend.entity;

import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.FetchType;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.JoinColumn;
import jakarta.persistence.ManyToOne;
import jakarta.persistence.Table;
import java.time.OffsetDateTime;

@Entity
@Table(name = "bomba_programming_history")
public class BombaProgrammingHistory {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "bomba_id", nullable = false)
    private Bomba bomba;

    @Column(name = "actuator_name", nullable = true, length = 100)
    private String actuatorName;

    @Column(name = "actuator_type", nullable = true, length = 20)
    private String actuatorType;

    @Column(name = "temperature_on", nullable = false)
    private Double temperatureOn;

    @Column(name = "temperature_off", nullable = false)
    private Double temperatureOff;

    @Column(name = "work_duration_seconds", nullable = false)
    private Integer workDurationSeconds;

    @Column(name = "recorded_at", nullable = false)
    private OffsetDateTime recordedAt = OffsetDateTime.now();

    protected BombaProgrammingHistory() {
    }

    public BombaProgrammingHistory(Bomba bomba, Double temperatureOn, Double temperatureOff, Integer workDurationSeconds) {
        this.bomba = bomba;
        this.actuatorName = bomba.getName();
        this.actuatorType = "BOMBA";
        this.temperatureOn = temperatureOn;
        this.temperatureOff = temperatureOff;
        this.workDurationSeconds = workDurationSeconds;
        this.recordedAt = OffsetDateTime.now();
    }

    public Long getId() {
        return id;
    }

    public Bomba getBomba() {
        return bomba;
    }

    public String getActuatorName() {
        return actuatorName;
    }

    public String getActuatorType() {
        return actuatorType;
    }

    public Double getTemperatureOn() {
        return temperatureOn;
    }

    public Double getTemperatureOff() {
        return temperatureOff;
    }

    public Integer getWorkDurationSeconds() {
        return workDurationSeconds;
    }

    public OffsetDateTime getRecordedAt() {
        return recordedAt;
    }
}
